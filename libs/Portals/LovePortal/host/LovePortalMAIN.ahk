...
LovePortalMain() {
    ...
    hostingFile := FileOpen("../../../Settings/LovePortal/host.txt", "r")
    PlayerpositionFile := FileOpen("../../../Settings/LovePortal/position.txt", "r")

    PlayerHost := hostingFile.ReadLine()
    hostingFile.Close()

    selectedPosition := PlayerpositionFile.ReadLine()
    PlayerpositionFile.Close()

    WorldLoaded := "none"  
    if PlayerHost == "true" {
        HostPickPortal(selectedPosition, PlayerHost)
    } else {
...
HostPickPortal(position, host) {
    ...
    while (true) { ; Continue until a portal is found
        SmoothMouseMove(baseX + (col * colSpacing), baseY + (row * rowSpacing))
        Sleep(500) 

        ; Check for either portal type
        foundPortal := false
        ...
        else if (FindText(&X, &Y, 1361-150000, 607-150000, 1361+150000, 607+150000, 0, 0, spiritPick)) {
            Sleep(1000)
            MouseGetPos(&mouseX, &mouseY)
            BetterClick(mouseX, mouseY)
            foundPortal := true
        ...
        if (foundPortal) {
            ...
            break ; Exit the main loop after handling a portal
        } else {
            ; Move to the next portal position
            col++
            if (col >= 6) {
                col := 0
                row++
